...
#define GHNewfeatureCount self.imageNames.count
...
-(instancetype)initWithFeatureImages:(NSArray*)imageNames
{
    if(self = [super init])
    {
...
- (void)viewDidLoad
{
    ...
    for (int i = 0; i<GHNewfeatureCount; i++) {
...
#warning 就算不主动添加子控件到scrollView中，scrollView内部还是可能会有一些子控件
    ...
    scrollView.contentSize = CGSizeMake(GHNewfeatureCount * scrollW, 0);
    ...
    pageControl.numberOfPages = GHNewfeatureCount;
    pageControl.backgroundColor = [UIColor redColor];
    pageControl.currentPageIndicatorTintColor = kRGBColor(253, 98, 42);
...
//    [shareBtn setImage:[UIImage imageNamed:@"gp_bg_btn"] forState:UIControlStateNormal];
//    [shareBtn setImage:[UIImage imageNamed:@"gp_bg_btn"] forState:UIControlStateSelected];
...
//    shareBtn.height = 30;
//    
//    shareBtn.gh_centerX = imageView.width * 0.5;
//    shareBtn.gh_centerY = imageView.height * 0.65;
//    
...
//    
//    startBtn.gh_centerX = shareBtn.gh_centerX;
//    startBtn.gh_centerY = imageView.height * 0.75;
//    
...
//    [imageView addSubview:startBtn];
    ...
    //test
    CGFloat startBtnWidth = 240;
    UIButton *startBtn = [[UIButton alloc] initWithFrame:CGRectMake((kWindowW-startBtnWidth)/2.0, kWindowH-120, startBtnWidth, 50)];
    [startBtn addTarget:self action:@selector(startClick) forControlEvents:UIControlEventTouchUpInside];
    startBtn.backgroundColor = [UIColor redColor];
    [startBtn setTitle:@"欢迎开启" forState:UIControlStateNormal];
    [imageView addSubview:startBtn];
...
- (void)startClick
{
    ...
    window.rootViewController = [[GHNavigationController alloc]initWithRootViewController:vc];
    
}

- (void)dealloc
{
    GHLog(@"HWNewfeatureViewController-dealloc");
}
